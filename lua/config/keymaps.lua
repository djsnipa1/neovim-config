...
map(
  { 'n', 'v' }, '<leader>mcf',
    'i--<Space>{{{<CR><Esc>0dwo--<Space>}}}<Esc>k',
    { desc = 'Lua Fold Block' }
)
...
map(
  { 'n', 'v' }, 'V', '15jzz', { desc = 'easily move down' }
)
...
map(
  'v', 'K', ':m \'<-2<CR>gv=gv', { desc = 'move block up' }
)
...
map(
  'n', '<leader>ud', Util.toggle_diagnostics,
    { desc = 'Toggle Diagnostics' }
)
local conceallevel = vim.o.conceallevel > 0 and
                       vim.o.conceallevel or 3
map(
  'n', '<leader>uc', function()
...
map(
  'n', '<leader>gG', function()
    Util.float_term(
      { 'lazygit' }, { esc_esc = false, ctrl_hjkl = false }
    )
